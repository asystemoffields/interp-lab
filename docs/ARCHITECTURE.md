...
`validate-attribution-graph` consumes a graph plus repeated or held-out path-patching JSONL and writes a validation report. It separates effect rows from control rows, computes target-latent effect size, control-adjusted specificity, effect/control ratio, sign consistency, confidence intervals, and assigns each candidate path a status plus machine-readable reason codes. With `--graph-out`, it writes a graph copy annotated with validation details on matching path edges and candidate paths, plus a Markdown digest of the annotated graph.
