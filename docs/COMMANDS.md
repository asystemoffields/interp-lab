# interp-lab command reference

The complete CLI and data-format reference. New here? Start with the [README](../README.md) quick start, then come back for the full surface.

For focused walkthroughs see also: [`GOLDEN_REAL_MODEL_DEMO.md`](GOLDEN_REAL_MODEL_DEMO.md), [`REAL_MODEL_DEMOS.md`](REAL_MODEL_DEMOS.md), [`GEMMA4_WALKTHROUGH.md`](GEMMA4_WALKTHROUGH.md), [`SCALING.md`](SCALING.md), [`PYTHON_API.md`](PYTHON_API.md), and [`ARCHITECTURE.md`](ARCHITECTURE.md).

## Global options

Every subcommand accepts `--text-embedder NAME`, which selects how text is turned into fingerprint vectors for matching, text-pivot, and feature search:

- `hash` (default) — dependency-free lexical token hashing. Deterministic and offline; matches shared words, not meaning.
- `minilm` — local semantic embeddings via sentence-transformers (`pip install "interp-lab[embeddings]"`). Free, offline, no API key.
- `st:<model-name>` — any other sentence-transformers model.
