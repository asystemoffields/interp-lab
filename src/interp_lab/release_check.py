...
def _check_version_sync(pyproject: dict[str, Any]) -> dict[str, str]:
    ...
    return _check(
        ...
        f"pyproject={project_version}, interp_lab={interp_version}",
        "Synchronize pyproject.toml and interp_lab.__version__ before release.",
    )
...
def _check_browser_app(root: Path) -> dict[str, str]:
    files = ["src/interp_lab/web_app.py", "src/interp_lab/web_server.py"]
    missing = [path for path in files if not (root / path).exists()]
    web_app = (root / "src/interp_lab/web_app.py").read_text(encoding="utf-8") if not missing else ""
    web_server = (root / "src/interp_lab/web_server.py").read_text(encoding="utf-8") if not missing else ""
