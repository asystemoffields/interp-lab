...
This writes JSON and Markdown summaries with effect sizes, control comparisons, sign consistency, confidence intervals, path status, claim grade, validation reason codes, and a next action. `--graph-out` writes a copy of the attribution graph with validation attached to matching path edges and candidate paths, plus a `validated-graph.md` digest by default.
